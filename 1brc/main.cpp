...
auto main(int argc, char *argv[]) -> int {
  ...
  std::vector<std::unordered_map<std::string_view, std::vector<float>>>
      result_map(num_procs);
  std::for_each(std::execution::par_unseq, chunks.begin(), chunks.end(),
                [&result_map](struct Chunk &chunk) {
                  ...
                  while (true) {
                    ...
                    const auto measurement =
                        std::stof(std::string{measurement_str});
  ...
  std::ranges::sort(result_vector,
                    [](const entry_type &pair1, const entry_type &pair2) {
                      return pair1.first < pair2.first;
                    });

  // get values
  for (const auto &[entry, value] : result_vector) {
    const auto min = *std::min_element(std::execution::par_unseq, value.begin(),
                                       value.end());
    const auto max = *std::max_element(std::execution::par_unseq, value.begin(),
                                       value.end());
